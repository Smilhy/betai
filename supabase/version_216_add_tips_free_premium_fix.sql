-- BetAI wersja 216 — naprawa dodawania kuponów/typów FREE i PREMIUM
-- Wklej w Supabase SQL Editor i uruchom jeden raz.
-- Skrypt jest bezpieczny: dodaje brakujące kolumny i podmienia tylko polityki RLS dla tabeli tips.
...
-- Uzupełnienie starych rekordów, żeby nie blokowały NOT NULL / widoku.
update public.tips
set
...
begin
  if not exists (
    ...
    where conname = 'tips_access_type_free_premium_check'
      and conrelid = 'public.tips'::regclass
  ) then
    alter table public.tips
      add constraint tips_access_type_free_premium_check
...
create policy "BetAI insert free premium tips"
on public.tips
for insert
to authenticated
with check (
  ...
  and access_type in ('free', 'premium')
);

create policy "BetAI update own tips"
on public.tips
for update
to authenticated
using (
